...
impl<'tcx> DeadCodeElimination<'tcx> {
    pub fn run(tcx: TyCtxt<'tcx>, def_id: DefId, body: &mut mir::Body<'tcx>) {
        ...
        // this visit will remove all edges from Goto blocks to unreachable targets.
        // Later this will lead to the compiler eliminating these blocks.
    ...
    fn remove_assertions(&self, body: &mut mir::Body<'tcx>) {
        ...
        for block in self.removable_assertions.iter() {
            // take the existing terminator, which has to be an assert or something
            // went very wrong.
            ...
            } = terminator.kind
            {
                ...
                if matches!(kind, AssertKind::Overflow(..) | AssertKind::OverflowNeg(_)) {
                    ...
                    if place.projection.len() == 1 {
                        // figure out the type of the first field (the one that
                ...
                // this only happens if our locations are inaccurate, which is
                // hopefully never if we order our modifications correctly
        ...
        // now we can replace the checked operations with unchecked ones, and if
        // let's say _x = CheckedAdd() is replaced with _y = Add(), we have to replace
        // every occurrence of _x.0 with _y
        // 1. create the new locals with the previously derived types
        ...
        // replace the operations and occurrences of locals
...
impl<'tcx> MutVisitor<'tcx> for OperationReplacer<'tcx> {
    ...
    fn visit_place(
        ...
        if let Some(replacing_local) = self.replacements.get(&place.local) {
            // if this is an access to _local.0, replace it
            if let mir::ProjectionElem::Field(id, _) = place.projection.first().unwrap() && id.index() == 0 {
