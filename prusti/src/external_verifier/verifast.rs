use std::{fs::File, io::{self, Read, Write}, iter::{Enumerate, Peekable}, str::Chars};
...
pub fn verify_external(translated_specs: TranslatedSpecMap) -> Vec<ExternalVerificationError> {
    translated_specs.into_iter()
    .filter(|(file, _)| file.ends_with(".c"))
    .flat_map(|(file, specs)| verify_external_c_file(&file, specs))
    .collect()
}

fn verify_external_c_file(file: &str, specs: FxHashMap<ExternFunctionName, ExternSpecification>) -> Vec<ExternalVerificationError> {
...
fn process_c_file(c_code: &mut String, specs: &FxHashMap<ExternFunctionName, ExternSpecification>) -> Vec<ExternalVerificationError> {
    specs.iter()
        .map(|(function_name, specification)| insert_specification(c_code, function_name, specification))
        .flat_map(Result::err)
        .collect()
}

fn insert_specification(c_code: &mut String, function_name: &ExternFunctionName, spec: &ExternSpecification) -> Result<(), ExternalVerificationError> {
    ...
    for (v_comments, c) in skip_over_c_comments(c_code[function_index + parameter_list_length..].chars()) {
    ...
    verifast_comments.iter().for_each(|(start, end)| {
        c_code.replace_range(function_index + parameter_list_length + start - offset..=function_index + parameter_list_length + end - offset, "");
        offset += end - start + 1;
    });
...
fn find_function(c_code: &str, function_name: &ExternFunctionName) -> Result<usize, ExternalVerificationError> {
    ...
    while let Some(match_index) = c_code[search_index..].find(function_name) {
        ...
        found_index = match found_index {
            Some(_) => return Err(ExternalVerificationError::AmbiguousExternFunctionName(function_name.clone())),
...
fn skip_over_c_comments<'a>(chars: Chars<'a>) -> impl Iterator<Item = (Vec<(usize, usize)>, char)> + 'a {
    struct Iter<'a> {
    ...
    Iter { chars: chars.enumerate().peekable() }
}