...
// The following post-condition may not hold because of the else block
...
// #[requires(list.len() > 4)]
// #[ensures(list.len() == old(list.len()))]
// #[ensures(forall (|i: usize| (0 <= i && i < old(list.len())) ==>
//     get(list, i) == old(get(list, i))))]
// fn test(list: &mut LinkedList<i32>) {
//     let mut tail = list.split_off(2);
//     list.append(&mut tail);
// }

/// Loop invariants are currently not
fn reverse(list: &mut LinkedList<i32>) {
    let mut stack = LinkedList::new();
    while !list.is_empty() {
        let first = list.pop_front();
        // Loop invariants currently not working
        if first.is_some() {
...
fn main() {
    ...
    list.push_back(23);
    let clist = clone(&mut list);
    assert!(get(&clist, 0) == get(&list, 0))
}
