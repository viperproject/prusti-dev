...
impl From<polymorphic::Type> for legacy::Type {
    fn from(typ: polymorphic::Type) -> legacy::Type {
        match typ {
            ...
            polymorphic::Type::Map(map) => legacy::Type::Map(
                Box::new((*map.key_type).into()),
                Box::new((*map.val_type).into()),
            ),
    ...
    fn from(expr: polymorphic::Expr) -> legacy::Expr {
        match expr {
            ...
            polymorphic::Expr::Map(map) => legacy::Expr::Map(
                map.typ.into(),
                map.elements.into_iter().map(Into::into).collect(),
                map.position.into(),
            ),
            ...
            polymorphic::Expr::BuiltinFuncApp(builtin_func_app) => {
                match builtin_func_app.function {
                    ...
                    polymorphic::BuiltinFunc::EmptyMap =>
                        return legacy::Expr::Map(
                            builtin_func_app.return_type.into(),
                            builtin_func_app
                                .arguments
                                .into_iter()
                ...
                let op_kind = match builtin_func_app.function {
                    ...
                    polymorphic::BuiltinFunc::UpdateMap => legacy::ContainerOpKind::MapUpdate,
                    polymorphic::BuiltinFunc::LookupMap => legacy::ContainerOpKind::MapIndex,
                    polymorphic::BuiltinFunc::SeqLen => legacy::ContainerOpKind::SeqLen,
                    polymorphic::BuiltinFunc::MapLen => legacy::ContainerOpKind::MapLen,
