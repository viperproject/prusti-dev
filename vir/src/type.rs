...
macro_rules! impl_exp_type {
    ...
    ($inner:ident$([$name:ident = $const:expr])?$( => $($up:tt)|+)?, $expected:pat$( if $cond:expr)?$( => $neg:literal)?, $doc:literal) => {
        ...
        unsafe impl TransmuteFrom<$inner> for $inner {}
        $(
            $(
...
// `Bool`, `Int`, `Perm` and the collection types are subtypes of `CSnap`
// since Rust's `bool` and the `prusti_contracts` builtins (`Int`, `Real`,
// `Set`, `Multiset`, `Seq`, `Map`) translate to these. The meaning
// of `Snap` is the mathematical representation of any Rust type (which these
// types are). Note that the `Prim` type is unrelated and other `Prim` types
// are not `Snap`.
// TODO: we could also have an `Impure` supertype of `Bool` which can contain `acc`s.
impl_exp_type!(Bool[TYPE_BOOL = Bool] => Prim | CSnap | Snap | Dyn, "The Viper `Bool` type");
impl_exp_type!(Int[TYPE_INT = Int] => Prim | CSnap | Snap | Dyn, "The Viper `Int` type");
impl_exp_type!(Perm[TYPE_PERM = Perm] => Prim | CSnap | Snap | Dyn, "The Viper `Perm` type (reals)");
impl_exp_type!(Ref[TYPE_REF = Ref] => Prim | Dyn, "The Viper `Ref` type");
impl_exp_type!(Set => Prim | CSnap | Snap | Dyn, TypeKind::Set(_), "The Viper `Set` type");
impl_exp_type!(Multiset => Prim | CSnap | Snap | Dyn, TypeKind::Multiset(_), "The Viper `Multiset` type");
impl_exp_type!(Seq => Prim | CSnap | Snap | Dyn, TypeKind::Seq(_), "The Viper `Seq` type");
impl_exp_type!(Map => Prim | CSnap | Snap | Dyn, TypeKind::Map(..), "The Viper `Map` type");

impl_exp_type!(Err[TYPE_ERR = Err] => Prim | Dyn, "Type for encoding errors");

impl_exp_type!(CSnap => Snap | Dyn, ref kind if matches!(kind, TypeKind::Bool | TypeKind::Int | TypeKind::Perm | TypeKind::Set(_) | TypeKind::Multiset(_) | TypeKind::Seq(_) | TypeKind::Map(..)) || matches!(kind, TypeKind::Domain(name, ..) if name.starts_with("s_") && *name != "s_Param"), "A concrete Prusti snapshot type");
...
impl_exp_type!(Snap => Dyn, ref kind if matches!(kind, TypeKind::Bool | TypeKind::Int | TypeKind::Perm | TypeKind::Set(_) | TypeKind::Multiset(_) | TypeKind::Seq(_) | TypeKind::Map(..)) || matches!(kind, TypeKind::Domain(name, ..) if name.starts_with("s_")), "A Prusti snapshot type, either concrete or generic");
