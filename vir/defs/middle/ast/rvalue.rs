pub(crate) use super::{
    ...
    statement::BlockMarkerCondition,
...
pub enum DiscriminantReadKind {
    /// Read out discriminant of an owned enum. Requires permission to the entire
    /// enum.
    Enum,
    /// Directly read the value of a discriminant field. Requires permission
    /// only to the field.
    Field,
}
...
#[display(
    fmt = "discriminant({}, {})",
    "display::option_foreach!(use_field, \"<{}>\", \"{};\", \"always\")",
    place
)]
pub struct Discriminant {
    /// Use field when this is None or any of the conditions holds.
    pub use_field: Option<Vec<BlockMarkerCondition>>,
    pub place: Expression,
