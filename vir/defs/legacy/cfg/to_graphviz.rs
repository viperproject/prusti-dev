...
use crate::config;
use std::io::Write;
use crate::legacy::{self, cfg::method::*};
...
impl CfgMethod {
    ...
    fn block_to_graphviz<'b>(
        ...
        header_rows: &[String],
        footer_rows: &[String],
    ) -> (String, Vec<&'b legacy::borrows::DAG>) {
        ...
        for stmt in &block.stmts {
            ...
            first_row = false;
            match stmt {
                legacy::Stmt::ExpireBorrows(ref dag) => {
                    reborrowing_dags.push(dag);
                }
                legacy::Stmt::PackageMagicWand(_, ref stmts, _, _, _) => {
                    for stmt in stmts {
                        if let legacy::Stmt::ExpireBorrows(ref dag) = stmt {
                            reborrowing_dags.push(dag);
                        }
                    }
                }
