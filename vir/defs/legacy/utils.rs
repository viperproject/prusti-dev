...
impl legacy::Stmt {
    ...
    where
        F: Fn(legacy::Expr) -> legacy::Expr,
    {
        trace!("Stmt::map_expr {}", self);
        struct StmtExprSubstitutor<T>
        where
            T: Fn(legacy::Expr) -> legacy::Expr,
        {
            substitutor: T,
        }
        impl<T> legacy::StmtFolder for StmtExprSubstitutor<T>
        where
            T: Fn(legacy::Expr) -> legacy::Expr,
        {
            fn fold_expr(&mut self, e: legacy::Expr) -> legacy::Expr {
                (self.substitutor)(e)
            }
        }
    ...
    where
        F: Fn(legacy::Expr) -> Result<legacy::Expr, E>,
    {
        trace!("Stmt::fallible_map_expr {}", self);
        struct StmtExprSubstitutor<T, U>
        where
            T: Fn(legacy::Expr) -> Result<legacy::Expr, U>,
        {
            substitutor: T,
        }
        impl<T, U> legacy::FallibleStmtFolder for StmtExprSubstitutor<T, U>
        where
            T: Fn(legacy::Expr) -> Result<legacy::Expr, U>,
        {
            type Error = U;

            fn fallible_fold_expr(&mut self, e: legacy::Expr) -> Result<legacy::Expr, U> {
                (self.substitutor)(e)
            }
        }
...
impl legacy::Expr {
    ...
    where
        F: Fn(&str, legacy::Expr) -> legacy::Expr,
    {
        trace!("Expr::map_old_expr {}", self);
        struct ExprOldExprSubstitutor<T>
        where
            T: Fn(&str, legacy::Expr) -> legacy::Expr,
        {
            substitutor: T,
        }
        impl<T> legacy::ExprFolder for ExprOldExprSubstitutor<T>
        where
            T: Fn(&str, legacy::Expr) -> legacy::Expr,
        {
...
impl legacy::Expr {
        ...
        F: Fn(String) -> String,
    {
        ...
        impl<T> legacy::ExprFolder for ExprLabelSubstitutor<T>
        where
            T: Fn(String) -> String,
        {
            fn fold_labelled_old(
                ...
                legacy::Expr::LabelledOld((self.substitutor)(x), y, p)
            }
        }
