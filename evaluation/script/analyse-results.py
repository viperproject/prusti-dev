...
ROOT = os.path.join(SCRIPTS_PATH, '..', '..')
ROSETTA_PATH = os.path.join(ROOT, 'evaluation', 'rosetta')
...
def main():
    ...
    # Traits:
    ignore_reason(results, 'trait types are not supported')
    ignore_reason(results, 'associated types are not supported')

    ignore_reason(results, 'shared raw pointers are partially supported')
