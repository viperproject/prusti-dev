...
/// These elements can be used in the ``Analyzer`` to represent an abstraction of the concrete state at program points
///
/// To ensure that the analysis is converging to a correct fixed point
/// implementations `S` of this trait should fulfill the following properties:
/// * `join()` implicitly defines a partial order of abstraction `<=`, such that\
///     forall x,y: S :: `x <= x.join(y)` && `y <= x.join(y)`;\
///     i.e. `join()` computes an upper bound in that order, which means it abstracts more (or equally many) concrete states,
///     in particular it represents all concrete states that were abstracted by either `x` or `y`
/// * `new_bottom()` should return the least element in that order
/// * If the 'height' of the order, i.e. how many elements can be traversed until a maximum is reached,
///   is not known to be finite, the following properties should additionally hold for proper widening:
...
///   i.e. the resulting abstraction should represent a superset of the possible concrete states after
///   applying the statement or terminator to any of the concrete states represented by the abstraction before
...
///    `SuccessorWithoutState` error will be returned by the analysis.
///
/// To get a result that is as precise as possible implementers probably also want to fulfill the following properties
/// * `join()` should return the **least** least upper bound,\
///    in particular it should hold:\
