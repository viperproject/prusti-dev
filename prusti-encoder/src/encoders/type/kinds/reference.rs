...
use crate::encoders::{domain::{DomainBuilder, DomainDataRef, DomainEnc, DomainEncSpecifics}, predicate::{PredicateBuilder, PredicateEncData, PredicateEncDataRef}, rust_ty_snapshots::RustTySnapshotsEnc, snapshot::SnapshotEncOutput, PredicateEnc};
...
pub(crate) fn domain<'vir>(
    ...
    let ty = task_key.ty();
    let ty_kind = ty.kind();
    let ty::TyKind::Ref(_, inner_ty, _) = ty_kind else { unreachable!(); };
...
pub(crate) fn predicate<'vir>(
    ...
    Ok((PredicateEncData::Ref(PredicateEncDataRef {
        deref_func: deref_func.to_known(),
        perm: None,
        snap_data,
    }), None/*builder.vcx.mk_lazy_expr("ref_indirect", |vcx, expr| {
        vcx.
    })*/))
}
