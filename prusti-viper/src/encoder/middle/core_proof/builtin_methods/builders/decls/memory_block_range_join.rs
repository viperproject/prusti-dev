...
impl<'l, 'p, 'v, 'tcx> MemoryBlockRangeJoinMethodBuilder<'l, 'p, 'v, 'tcx> {
    ...
    pub(in super::super::super::super) fn add_byte_values_preserved_postcondition(
        ...
        let element_bytes = self
            ...
            .inner
            .lowerer
            .encode_memory_block_bytes_expression(element_address.clone(), element_size.clone())?;
        let whole_bytes = self
            .inner
            .inner
            .lowerer
            .encode_memory_block_bytes_expression(address.clone(), whole_size.clone())?;
        let read_element_byte = self.inner.inner.lowerer.encode_read_byte_expression(
        ...
        let trigger = self.inner.inner.lowerer.encode_read_byte_expression(
            element_bytes.clone(),
            byte_index.clone().into(),
            self.inner.inner.position,
        )?;
