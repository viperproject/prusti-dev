...
use crate::encoder::{
    ...
    middle::core_proof::{
        ...
        snapshots::{
            ...
            ProcedureExpressionToSnapshot, SnapshotVariablesInterface,
        },
...
impl<'p, 'v: 'p, 'tcx: 'v> AddressesInterface for Lowerer<'p, 'v, 'tcx> {
    ...
    fn address_offset(
        ...
        if !self.address_state.is_address_offset_axiom_encoded {
            ...
            self.declare_axiom("Address", axiom)?;
        }
        self.create_domain_func_app(
            "Address",
            "offset_address$",
        ...
        let result = match place {
            ...
            vir_mid::Expression::Deref(deref) => {
                ...
                let mut place_encoder =
                    ProcedureExpressionToSnapshot::for_address(PredicateKind::Owned);
                let base_snapshot =
    ...
    fn encode_index_address(
        ...
        self.address_offset(size, start_address, index.clone().into(), position)
    }
}
