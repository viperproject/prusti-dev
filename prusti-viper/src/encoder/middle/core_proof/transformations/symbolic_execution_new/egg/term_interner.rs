...
impl TermInterner for EGraph<ExpressionLanguage, ()> {
    ...
    fn intern_term(
        ...
        Ok(
            try_intern_term_rec(self, true_id, false_id, &BTreeMap::new(), term)
    ...
    fn intern_pattern(
        ...
        Ok(
            try_intern_term_rec(self, true_id, false_id, variables, term)
                .unwrap_or_else(|| panic!("Failed to intern term: {term}")),
        )
    }

    fn add(&mut self, term: ExpressionLanguage) -> Id {
        assert!(self.are_explanations_enabled());
...
fn try_intern_term_rec(
    ...
    let id = match term {
        ...
        vir_low::Expression::UnaryOp(expression) => {
            let operand_id =
                try_intern_term_rec(egraph, true_id, false_id, variables, &expression.argument)?;
        ...
        vir_low::Expression::BinaryOp(expression) => {
            let left_id =
                try_intern_term_rec(egraph, true_id, false_id, variables, &expression.left)?;
            let right_id =
                try_intern_term_rec(egraph, true_id, false_id, variables, &expression.right)?;
            match expression.op_kind {
        ...
        vir_low::Expression::ContainerOp(expression) => {
            ...
            for operand in &expression.operands {
                let operand_id =
                    try_intern_term_rec(egraph, true_id, false_id, variables, operand)?;
        ...
        vir_low::Expression::DomainFuncApp(expression) => {
            ...
            let arguments = expression
                .arguments
                .iter()
                .map(|argument| try_intern_term_rec(egraph, true_id, false_id, variables, argument))
                .collect::<Option<Vec<_>>>()?;
            egraph.add(ExpressionLanguage::FuncApp(symbol, arguments))
        }
        vir_low::Expression::LabelledOld(expression) => {
            try_intern_term_rec(egraph, true_id, false_id, variables, &expression.base)?
        }
