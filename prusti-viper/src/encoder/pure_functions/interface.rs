...
use crate::encoder::{encoder::SubstMap, errors::SpannedEncodingResult, stub_function_encoder::StubFunctionEncoder};
...
impl<'v, 'tcx: 'v> PureFunctionEncoderInterface<'tcx> for super::super::Encoder<'v, 'tcx> {
   fn encode_pure_expression(
            ...
            if !self.pure_function_encoder_state.pure_function_bodies.borrow().contains_key(&key) {
                ...
                self.pure_function_encoder_state.pure_function_bodies
                    .borrow_mut()
                    .insert(key.clone(), body);
            }
            Ok(self.pure_function_encoder_state.pure_function_bodies.borrow()[&key].clone())
        }
    ...
    fn encode_pure_function_def(
        ...
        if !self.pure_function_encoder_state.pure_functions.borrow().contains_key(&key) &&
                !self.pure_function_encoder_state.pure_functions_encoding_started.borrow().contains(&key) {
            trace!("not encoded: {:?}", key);

            self.pure_function_encoder_state.pure_functions_encoding_started.borrow_mut().insert(key.clone());
            ...
            match maybe_identifier {
                Ok(identifier) => {
                    self.pure_function_encoder_state.pure_functions.borrow_mut().insert(key, identifier);
                }
                Err(error) => {
