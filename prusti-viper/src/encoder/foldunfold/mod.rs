...
impl<'p, 'v: 'p, 'tcx: 'v> FoldUnfold<'p, 'v, 'tcx> {
    ...
    fn restore_write_permissions(
        ...
        for access in pctxt.log().get_converted_to_read_places(borrow) {
            ...
            let inhale_stmt = polymorphic_vir::Stmt::Inhale(access);
        ...
        if let Some(label) = maybe_label {
            ...
            stmts
                ...
                .map(|stmt| match stmt {
                    ...
                    polymorphic_vir::Stmt::Unfold( polymorphic::Unfold {ref predicate_name, ref arguments, permission, enum_variant} ) => {
    ...
    for FoldUnfold<'p, 'v, 'tcx>
{
    ...
    fn replace_stmt(
        ...
        match stmt {
            ...
            | polymorphic_vir::Stmt::Inhale( polymorphic_vir::Inhale {expr: magic_wand, ..} ) => {
                if let polymorphic_vir::Expr::MagicWand (polymorphic_vir::MagicWand {box ref left, ..} ) = magic_wand {
                    ...
                    polymorphic_vir::Stmt::Inhale(lhs.clone())
        ...
        stmt = match stmt {
            polymorphic_vir::Stmt::If( polymorphic::If {guard, then_stmts, else_stmts} ) => {
...
impl<'b, 'a: 'b> FallibleExprFolder for ExprReplacer<'b, 'a> {
    ...
    fn fallible_fold_magic_wand(
        ...
        let res = polymorphic_vir::Expr::MagicWand(new_lhs, new_rhs, borrow, pos);
