...
impl Snapshot {
    ...
    pub fn encode_equals(&self, lhs: vir::Expr, rhs: vir::Expr, _pos: vir::Position) -> vir::Expr {
        vir::Expr::eq_cmp(self.snap_call(lhs), self.snap_call(rhs))
    }

    pub fn encode_not_equals(&self, lhs: vir::Expr, rhs: vir::Expr, _pos: vir::Position) -> vir::Expr {
...
impl<'p, 'v, 'r: 'v, 'a: 'r, 'tcx: 'a> SnapshotEncoder<'p, 'v, 'tcx> {
    ...
    fn encode_valid_axiom(&self, _cons_func: vir::DomainFunc) -> EncodingResult<vir::DomainAxiom> {
        ...
        let expr = vir::Expr::forall(vec![self_var], vec![],  vir::Expr::eq_cmp(self_valid_function_app, valid_func_apps)); //TODO triggers
        Ok(vir::DomainAxiom {
    ...
    fn get_ref_field(&self, var: vir::LocalVar) -> vir::Expr {
        self.encoder.insert_raw_ref_field("val_ref");
        vir::Expr::field(
            vir::Expr::local(var),
    ...
    fn variant_index(&self, variant: Option<&ty::VariantDef>) -> usize {
        if let Some(v) = variant {
            self.adt_def
                ...
                .iter()
                .enumerate()
                .find(|(_id, var)| var.ident == v.ident)
                .unwrap()
                .0
        } else {
            0
