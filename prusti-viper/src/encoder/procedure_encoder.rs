...
pub struct ProcedureEncoder<'p, 'v: 'p, 'tcx: 'v> {
    ...
    /// magic wands from array accesses
    array_magic_wand_at: HashMap<mir::Location, (vir::Expr, vir::Expr, vir::Expr, vir::Expr, Option<vir::borrows::Borrow>)>,
...
impl<'p, 'v: 'p, 'tcx: 'v> ProcedureEncoder<'p, 'v, 'tcx> {
    ...
    fn encode_loan_places(&mut self, loan_places: &LoanPlaces<'tcx>) -> EncodingResult<(vir::Expr, vir::Expr, bool, Vec<vir::Stmt>)> {
        ...
        if let mir::Rvalue::Ref(_, mir::BorrowKind::Mut { .. }, ref rhs_place) = loan_places.source {
            let (rhs_place_encoding, rhs_ty, _) = self.mir_encoder.encode_place(rhs_place).unwrap();
            if let PlaceEncoding::ArrayAccess { .. } = rhs_place_encoding {
                // encode expiry of the array borrow
                let (wand_lhs, expired_expr, wand_rhs, regained_expr, borrow) = self.array_magic_wand_at[&loan_places.location].clone();
                ...
                // // NOTE: there will be a Stmt::TransferPerm inserted after the statements we
                // // construct here by construct_vir_reborrowing_node_for_assignment, so instead of
                // // [expiring_base] --* [wand_rhs]
                // // we just want something like
                // // inhale [wand_rhs without regained_expr]
                // let wand_rhs_without_regained_expr = if let vir::Expr::BinOp(vir::BinOpKind::And, box vir::Expr::PredicateAccessPredicate(..), box rest_of_rhs, pos) = wand_rhs_patched_lhs { rest_of_rhs } else { wand_rhs_patched_lhs };

                let (arr_pred, arr_contents) = if let vir::Expr::BinOp(vir::BinOpKind::And, box lhs, box rhs, ..) = wand_rhs_patched_lhs { (lhs, rhs) } else { unreachable!() };
                ...
                stmts.push(vir!{ inhale [arr_contents] });
                ...
                // would fail.
                return Ok((expiring_base, regained_expr, false, stmts));
            }
        }
    ...
    fn encode_terminator(
        ...
        let result = match term.kind {
            ...
            TerminatorKind::DropAndReplace {
                ...
                // will panic if attempting to encode unsupported type
                // TODO: ignore stmts here as well?
                let (encoded_lhs, pre_stmts, _, _) = self.encode_place(lhs, ArrayAccessKind::Mutable(None, location)).unwrap();
                stmts.extend(pre_stmts);
                stmts.extend(
    ...
    fn encode_assign_array_repeat_initializer(
        ...
        for i in 0..len {
            ...
            let lookup_pure_call = array_types.encode_lookup_pure_call(encoded_lhs.clone(), idx);
    ...
    fn encode_array_lookup_mut(
        ...
        let array_access_pred = vir::Expr::predicate_access_predicate(
            at.array_pred.clone(),
            encoded_base_expr.clone(),
            vir::PermAmount::Write,
        );
        ...
        // value of res
        stmts.push(vir::Stmt::Inhale(
            vir!{ [ old(at.encode_lookup_pure_call(encoded_base_expr.clone(), idx_val_int.clone())) ] == [ res_val_field ] }
        ));
        ...
        let ref_expr = self.mir_encoder.encode_place_predicate_permission(res.clone(), vir::PermAmount::Write).unwrap();

        let magic_wand_rhs = vir!{ [array_access_pred] && ([all_others_unchanged] && [indexed_updated]) };
        // let magic_wand_rhs = vir!{ [array_access_pred] && [all_others_unchanged] };
        self.array_magic_wand_at.insert(
            location,
            (ref_expr, res_val_field, magic_wand_rhs, encoded_base_expr, loan.clone())
        );

        // let magic_wand = vir!{
        //     // [ref_expr] { loan } --* [magic_wand_rhs]
        //     [res_val_field] { loan } --* [magic_wand_rhs]
        // };
        // stmts.push(vir!{ inhale [magic_wand] });
