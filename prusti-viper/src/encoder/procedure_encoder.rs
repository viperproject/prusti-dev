...
pub struct ProcedureEncoder<'p, 'v: 'p, 'r: 'v, 'a: 'r, 'tcx: 'a> {
    ...
    /// Magic wand generated by a call at a given location.
    magic_wand_at_location: HashMap<mir::Location, (vir::Expr, vir::Expr)>,
}
...
impl<'p, 'v: 'p, 'r: 'v, 'a: 'r, 'tcx: 'a> ProcedureEncoder<'p, 'v, 'r, 'a, 'tcx> {
            ...
            mir::StatementKind::Assign(ref lhs, ref rhs) => {
                ...
                match rhs {
                    ...
                    &mir::Rvalue::NullaryOp(op, ref op_ty) => {
                        match op {
                            mir::NullOp::Box => {
                                ...
                                let box_content = encoded_lhs.clone().access_curr(ref_field);
                        ...
                        match src_ty.sty {
                            ty::TypeVariants::TyAdt(ref adt_def, _) if !adt_def.is_box() => {
                                ...
                                let discr_value: vir::Expr = if num_variants <= 1 {
                                    ...
                                    self.translate_maybe_borrowed_place(
                                        location,
                                        encoded_src.access_curr(discr_field)
                                    ).into()
                                };
                                let int_field = self.encoder.encode_value_field(ty);
                                stmts.push(
                                    vir::Stmt::Assign(
                                        encoded_lhs.clone().access_curr(int_field),
                                        discr_value,
                                        vir::AssignKind::Copy
                                    )
                                );
                            ...
                            ty::TypeVariants::TyUint(_) => {
                                ...
                                let discr_value: vir::Expr = self.translate_maybe_borrowed_place(
                                    location,
                                    encoded_src.access_curr(value_field)
                                ).into();
                                let int_field = self.encoder.encode_value_field(ty);
                                stmts.push(
                                    vir::Stmt::Assign(
                    ...
                    &mir::Rvalue::Ref(ref _region, _, ref place) => {
                        ...
                        stmts.push(
                            vir::Stmt::Assign(
                                encoded_lhs.clone().access_curr(ref_field),
                                encoded_value.into(),
    ...
    fn encode_loan_places(&self, loan_places: &LoanPlaces<'tcx>) -> (vir::Place, vir::Place) {
        ...
        match loan_places.source {
            mir::Rvalue::Ref(_, _, ref rhs_place) => {
                ...
                let expiring = expiring_base.clone().access_curr(ref_field);
            ...
            mir::Rvalue::Use(mir::Operand::Move(ref rhs_place)) => {
                ...
                let ref_field = self.encoder.encode_value_field(expiring_ty);
                let expiring = expiring_base.clone().access_curr(ref_field.clone());
                let restored = restored_base.clone().access_curr(ref_field);
                assert_eq!(expiring.get_type(), restored.get_type());
                (expiring, restored)
            }
    ...
    fn encode_transfer_permissions(&mut self, lhs: vir::Place, rhs: vir::Place, location: mir::Location) -> Vec<vir::Stmt> {
        let mut stmts = vec![];
    ...
    fn encode_expiration_of_reborrowing_dag_node(&mut self, dag: &ReborrowingDAG, node: &ReborrowingDAGNode) -> Vec<vir::Stmt> {
        ...
        match node.kind {
            ...
            ReborrowingKind::ArgumentMove { ref loan } => {
                ...
                let encoded_ref = vir::Place::base(encoded_local).access_curr(ref_field);
                ...
                let lhs_place = if node.incoming_zombies {
                    ...
                    encoded_ref.clone().old(&lhs_label)
                } else {
                    encoded_ref.clone()
                };
            ...
            ReborrowingKind::Call { ref loan, ref variable, ref region } => {
                let loan_location = self.polonius_info.get_loan_location(&loan);
                let (lhs, rhs) = self.magic_wand_at_location.get(&loan_location).cloned().unwrap();
            ...
            TerminatorKind::Call {
                ...
                match func_proc_name {
                    ...
                    "<std::boxed::Box<T>>::new" => {
                        ...
                        let box_content = dst.clone().access_curr(ref_field);
                            ...
                            for operand in args.iter() {
                                ...
                                match (operand_place, &operand_ty.sty) {
                                    ...
                                    (Some(ref place), ty::TypeVariants::TyRef(_, ref inner_ty, _)) => {
                                        ...
                                        let ref_place = place.clone().access_curr(ref_field);
                                        stmts.extend(
                                            self.encode_transfer_permissions(
                                                ref_place.clone(),
                            ...
                            for operand in args.iter() {
                                ...
                                match self.mir_encoder.encode_operand_place(operand) {
                                    ...
                                    None => {
                                        ...
                                        fake_exprs.insert(fake_arg_place.access_curr(val_field), arg_val_expr);
                                    }
                                }
                            }
    ...
    fn encode_postcondition_expr(
        ...
        pre_label: &str,
        post_label: &str,
        magic_wand_store_info: Option<(mir::Location, &HashMap<vir::Place, vir::Expr>)>,
        diverging: bool
    ) -> (vir::Expr, vir::Expr) {
        ...
        if let Some((lhs, rhs)) = self.encode_postcondition_magic_wand(contract, pre_label, post_label) {
            let (location, fake_exprs) = magic_wand_store_info.unwrap();
            type_spec.push(vir::Expr::MagicWand(box lhs.clone(), box rhs.clone()));
            let replace_fake_exprs = |mut expr: vir::Expr| -> vir::Expr {
                for (fake_arg, arg_expr) in fake_exprs.iter() {
                    expr = expr.substitute_place_with_expr(&fake_arg, arg_expr.clone());
                }
            ...
            let lhs = replace_fake_exprs(lhs);
            let rhs = replace_fake_exprs(rhs);
            debug!("Insert ({:?} {:?}) at {:?}", lhs, rhs, location);
            self.magic_wand_at_location.insert(location, (lhs, rhs));
        }
            ...
            &mir::Operand::Constant(box mir::Constant { ty, ref literal, .. }) => {
                ...
                match literal {
                    mir::Literal::Value { value } => {
                        ...
                        stmts.push(
                            vir::Stmt::Assign(lhs.clone().access_curr(field), const_val, vir::AssignKind::Copy)
                        );
                    }
        ...
        match self_ty.sty {
            ...
            ty::TypeVariants::TyChar => {
                ...
                vec![
                    // Copy value
                    vir::Stmt::Assign(
                        dst.clone().access_curr(field.clone()),
                        src.clone().access_curr(field.clone()).into(),
                        vir::AssignKind::Copy
                    )
                ]
            }
            ...
            ty::TypeVariants::TyTuple(elems) => {
                ...
                for (field_num, ty) in elems.iter().enumerate() {
                    ...
                    stmts.extend(
                        // Copy fields
                        self.encode_copy(
                            src.clone().access_curr(field.clone()),
                            dst.clone().access_curr(field.clone()),
                            ty,
                            is_move,
                            location
            ...
            ty::TypeVariants::TyAdt(ref adt_def, ref subst) if !adt_def.is_box() => {
                ...
                if num_variants > 1 {
                    stmts.push(
                        // Copy discriminant
                        vir::Stmt::Assign(
                            dst.clone().access_curr(discriminant_field.clone()),
                            src.clone().access_curr(discriminant_field.clone()).into(),
                            vir::AssignKind::Copy
                        )
                    );
                }
                ...
                for (variant_index, variant_def) in adt_def.variants.iter().enumerate() {
                    for field in &variant_def.fields {
                        ...
                        stmts.extend(
                            // Copy fields
                            self.encode_copy(
                                src.clone().access_curr(elem_field.clone()),
                                dst.clone().access_curr(elem_field.clone()),
                                field_ty,
                                is_move,
                                location
                            )
            ...
            ty::TypeVariants::TyRef(_, ty, _) => {
                ...
                vec![
                    ...
                    vir::Stmt::Assign(
                        dst.clone().access_curr(ref_field.clone()),
                        src.clone().access_curr(ref_field.clone()).into(),
                        vir::AssignKind::Move
                    )
                ]
            }
        ...
        match aggregate {
            &mir::AggregateKind::Tuple => {
                ...
                for (field_num, operand) in operands.iter().enumerate() {
                    ...
                    stmts.extend(
                        self.encode_assign_operand(&dst.clone().access_curr(encoded_field), operand, location)
                    );
                }
                stmts
            }
            ...
            &mir::AggregateKind::Adt(adt_def, variant_index, subst, _) => {
                ...
                if num_variants > 1 {
                    ...
                    stmts.push(
                        vir::Stmt::Assign(
                            dst.clone().access_curr(discr_field).into(),
                            discr_value.into(),
                            vir::AssignKind::Copy
                        )
                    );
