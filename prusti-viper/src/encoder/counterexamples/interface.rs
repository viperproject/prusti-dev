use vir_crate::{low::{self as vir_low}, common::check_mode::CheckMode};
...
impl MirProcedureMapping{
            ...
            | basic_block | {
                ...
                for statement in &basic_block.statements{
                    match &statement {
                        ...
                        vir_low::Statement::Assert(_) => stmts.push(statement.clone()), //all statements that could be relevant for variable assignments
                        vir_low::Statement::Inhale(inhale) => { //all statements that could be relevant for pure function calls
                            if let Some(assume_stmt) = self.extract_domain_functions(&inhale.expression){
    ...
    fn extract_domain_functions (&self, expression: &vir_low::Expression) -> Option<vir_low::Statement>{
        ...
        //inhale destructor_Bool(constructor_Bool_EqCmp_*(snapvar_1 caller_for$m_foo$(snapvar_2)))
         match &expression {
            vir_low::Expression::BinaryOp(binary_op) => self.extract_domain_functions(&binary_op.right),
            vir_low::Expression::DomainFuncApp(domain_function_call) => {
                if domain_function_call.function_name.contains("valid$"){
                    if let Some(local) = domain_function_call.arguments.first(){
                        return Some(vir_low::Statement::Assume(vir_low::ast::statement::Assume{
...
impl<'v, 'tcx: 'v> MirProcedureMappingInterface for super::super::Encoder<'v, 'tcx> {
    fn add_mapping(&mut self, program: &vir_low::Program) {
        if let Some(vir_low_procedure) = program.procedures.first(){ 
            ...
            if matches!(program.check_mode, CheckMode::Specifications  ) || matches!(program.check_mode, CheckMode::Both) { 
