...
use vir_low::ast::{statement::Assume, expression::{BinaryOp, ContainerOp, DomainFuncApp, FuncApp, BinaryOpKind, ContainerOpKind}};
...
impl<'ce, 'tcx, 'v> VarMappingInterface for super::counterexample_translation_refactored::CounterexampleTranslator<'ce, 'tcx, 'v> {
            ...
            for basic_block in mir_procedure_mapping{
                ...
                for statement in &basic_block.stmts{
                    ...
                    let pure_fn_option = match statement{
                        vir_low::Statement::Assume(assume ) => self.extract_pure_fn_from_assume(assume),
                        _ => None,
                    };
    ...
    fn extract_pure_fn_from_assume(&self, statement: &Assume) -> Option<PureFunction> {
        ...
        match &statement.expression {
            vir_low::Expression::DomainFuncApp(DomainFuncApp{domain_name: _, arguments, .. }) => {
                if arguments.len() == 1 || arguments.len() == 2{ // might be a pure function call
                    return self.extract_pure_fn_from_assume(&Assume{expression: arguments.last().unwrap().clone(), position: vir_low::Position::default()});
                }
                None
            },
