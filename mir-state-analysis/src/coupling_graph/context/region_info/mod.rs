...
use prusti_rustc_interface::{
    borrowck::{
        borrow_set::{BorrowData, TwoPhaseActivation, BorrowSet},
...
impl<'tcx> RegionInfo<'tcx> {
    pub fn new(
        ...
        let mut map = RegionInfoMap::new(
            ...
            region_inference_context.var_infos.len(),
        );
        ...
        let (static_region, function_region) =
            Self::initialize_universals(&mut map, rp, input_facts, region_inference_context);

        // Init consts
        Self::initialize_consts(&mut map, rp, region_inference_context, borrow_set);

        // Init locals
        Self::initialize_locals(&mut map, rp, input_facts, borrow_set, sbs);

        // Init from `var_infos`
        for r in map.all_regions() {
            let info = region_inference_context.var_infos[r];
    ...
    pub fn initialize_universals(
        ...
        for c in region_inference_context
    ...
    pub fn initialize_locals(
        ...
        for data in sbs
            .location_map
            .values()
            .chain(borrow_set.location_map.values())
        {
...
struct ConstantRegionCollector<'a, 'b, 'tcx> {
    ...
    region_inference_context: &'b RegionInferenceContext<'tcx>,
    borrow_set: &'b BorrowSet<'tcx>,
    ...
    fn visit_assign(&mut self, place: &MirPlace<'tcx>, rvalue: &Rvalue<'tcx>, location: Location) {
        match rvalue {
            ...
            &Rvalue::Ref(region, kind, borrowed_place) => {
                ...
                let location_map = &self.borrow_set.location_map;
