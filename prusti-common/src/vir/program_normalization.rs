...
impl NormalizationInfo {
    ...
    pub fn normalize_program(program: &mut Program) -> Self {
        match program {
            ...
            Program::Legacy(legacy_program) => {
                ...
                let normalization_map: FxHashMap<u64, u64> = original_position_ids.iter().copied()
                    .enumerate()
    ...
    pub fn denormalize_program(&self, program: &mut Program) {
        match program {
            ...
            Program::Legacy(legacy_program) => {
                legacy_program.visit_expressions_mut(|e| e.visit_positions_mut(|p| *p = self.denormalize_position(*p)));
            }
        }
    }
    ...
    pub fn denormalize_position_string(&self, pos: &mut String) {
        let pos_id: u64 = pos.parse()
            .unwrap_or_else(|err| panic!("Cannot denormalize position {:?}: parsing error {:?}", pos, err));
