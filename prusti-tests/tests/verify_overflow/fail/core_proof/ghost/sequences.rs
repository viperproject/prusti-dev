...
fn empty_seq_zero_len() {
    prusti_assert!(Seq::new().len() == Int::from(0));
}

fn empty_seq_not_one_len() {
    prusti_assert!(Seq::new().len() == Int::from(1)); //~ ERROR: asserted expression might not hold
}

fn seq_eq1() {
    prusti_assert!(Seq::new() == Seq::new());
    prusti_assert!(Seq::single(1) == Seq::single(1));
    prusti_assert!(
        Seq::append(Seq::single(1), Seq::single(2)) == Seq::append(Seq::single(1), Seq::single(2))
    );
}
...
fn seq_construction() {
    let seq: pc::Seq<pc::Seq<Int>> = pc::Seq::new();
}
...
fn seq_macro() {
    prusti_assert!(seq![1, 2] == Seq::append(Seq::single(1), Seq::single(2)));
}
