...
fn sum_of_different_primes(primes: &VecWrapperI32, n: isize, k: isize) -> isize {
    ...
    while idx_k < size_k {
        ...
        helper(size_n, size_k, idx_k, &mut dp, primes);
        idx_k += 1;
    }
    dp.lookup(k, n)
}
...
#[requires(dp.y_size() == size_k && dp.x_size() == size_n)]
#[requires(idx_k > 0)]
#[requires(forall(|i: isize, j: isize| (i >= 0 && i < idx_k && j >= 0 && j < size_n) ==>  dp.lookup(i, j) == sum_of_different_primes_rec(primes, j, i)))]
#[requires(forall(|i: isize| (i >= 0 && i < size_n) ==>  dp.lookup(idx_k, i) == 0))]
#[ensures(forall(|i: isize, j: isize| (i >= 0 && i <= idx_k && j >= 0 && j < size_n) ==>  dp.lookup(i, j) == sum_of_different_primes_rec(primes, j, i)))]
fn helper(size_n: isize, size_k: isize, idx_k: isize, dp: &mut Matrix, primes: &VecWrapperI32) {
    ...
    while idx_n < size_n {
        ...
        dp.set(
            idx_k,
            idx_n,
            sum_of_different_primes_rec(primes, idx_n, idx_k),
        );
        idx_n += 1;
    }
}
