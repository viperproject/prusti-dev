...
#[requires(idx_k > 0 && idx_k <= k)]
#[requires(idx_n >= 0 && idx_n <= n)]
#[requires(dp.y_size() ==  k + 1)]
#[requires(dp.x_size() ==  n + 1)]
#[requires(forall(|i: isize, j: isize| (i >= 0 && i < idx_k && j >= 0 && j <= n) ==>  dp.lookup(i, j) == sum_of_different_primes_rec(primes, j, i)))]
#[requires(primes.len() >= 1)]
#[ensures(result == sum_of_different_primes_rec_helper(primes, idx_n, idx_k, primes.len() - 1))]
fn sum_of_different_primes_helper(primes: &VecWrapperI32, n: isize, k: isize, idx_n: isize, idx_k: isize, dp: &Matrix) -> isize {
