...
fn helper(
    ...
    if index_before == -1 {
        ...
        if index_weight >= weights.lookup(index_object) {
            ...
            assert!(dp.lookup(remaining_weight, index_object - 1) == solve_rec(prices, weights, index_object - 1, remaining_weight));
...
fn super_sale(
    ...
    while index_person < g {
        ...
        assert!(cur_answer == solve_rec(prices, weights, prices.len() -  1, w));
