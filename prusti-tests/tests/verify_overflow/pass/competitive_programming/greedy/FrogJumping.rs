...
fn solve_rec(positions: &VecWrapperI32, last_positions: &VecWrapperI32, idx: isize) -> isize{
    if idx == positions.len() - 1 {
        ...
        helper(positions, last_positions, idx + 1, last_positions.lookup(idx)) + 1
    }
}
...
fn helper(positions: &VecWrapperI32, last_positions: &VecWrapperI32, idx: isize, end: isize) -> isize {
    if idx == end {
        solve_rec(positions, last_positions, idx)
    } else {
        min(solve_rec(positions,  last_positions, idx), helper(positions, last_positions, idx + 1, end))
    }
}
...
#[requires(forall(|i: isize, j: isize| (i >= 0 && i < positions.len() - 1 && j > i && j < positions.len()) ==>
        (last_positions.lookup(i) <= last_positions.lookup(j))))]
#[ensures (forall(|i: isize| (i >= idx && i <= end) ==> helper(positions, last_positions, idx, end) <= solve_rec(positions,  last_positions, i)))]
#[ensures (helper(positions, last_positions, idx, end) == solve_rec(positions, last_positions, end))]
fn lemma(positions: &VecWrapperI32, last_positions: &VecWrapperI32, idx: isize, end: isize) {
    if idx == end {
        // solve_rec(positions, last_positions, idx)
    } else {
        lemma2(positions,  last_positions, idx);
        lemma(positions, last_positions, idx + 1, end);
        assert!(idx < end);
        assert!(helper(positions, last_positions, idx, end) == min(solve_rec(positions,  last_positions, idx), helper(positions, last_positions, idx + 1, end)))
    }
}
...
#[requires(forall(|i: isize, j: isize| (i >= 0 && i < positions.len() - 1 && j > i && j < positions.len()) ==>
        (last_positions.lookup(i) <= last_positions.lookup(j))))]
#[requires(last_positions.lookup(positions.len() - 1) == positions.len() - 1)]
#[ensures(forall(|i: isize, j:  isize| (i >= idx && i < positions.len() - 1 && j > i && j < positions.len()) ==> 
                solve_rec(positions, last_positions, i) >= solve_rec(positions, last_positions, j)))]
#[ensures(forall(|i: isize| (i >= idx && i < positions.len() - 1) ==> solve_rec(positions, last_positions, i) >= solve_rec(positions, last_positions, i + 1)))]
fn lemma2(positions: &VecWrapperI32, last_positions: &VecWrapperI32, idx: isize) {
    if idx == positions.len() - 1 {
        //0
    } else {
        assume_false();
        // lemma(positions, last_positions, idx + 1, last_positions.lookup(idx));
        //helper(positions, last_positions, idx + 1, last_positions.lookup(idx)) + 1
    }
}
...
//     while idx1 >= 0  {
//         body_invariant!(idx1 >= 0 && idx1 < positions.len() - 1);
//         body_invariant!(result.len() == positions.len());
//         body_invariant!(forall(|i: isize| (i > idx1 && i < positions.len() - 1) ==> (result.lookup(i) > i && result.lookup(i) < positions.len())));
//         // body_invariant!(forall(|i: isize, j: isize| (i > idx1 && i < positions.len() - 1 && j > result.lookup(i) && j < positions.len()) ==> 
//         //                 positions.lookup(j) - positions.lookup(i) > r));
//         let mut idx2 = idx1 + 1;
//         while idx2 < positions.len() && positions.lookup(idx2) - positions.lookup(idx1) <= r {
//             body_invariant!(idx2 > idx1 && idx2 < positions.len());
//             body_invariant!(true);
//             idx2 += 1;
//         }
//         assert!(idx2 == positions.len() || positions.lookup(idx2) - positions.lookup(idx1) > r);
