...
impl VecWrapper {
    ...
    // if index is not i, element stays untouched
    #[insert_runtime_check]
    #[ensures(forall(|j: usize| (j < self.len()) && j!=i ==> self.lookup(j) == old(self.lookup(j))))]
    // there is now an element with value x
    #[insert_runtime_check]
    #[ensures(exists(|j: usize| (j < self.len()) ==> self.lookup(j) == x))]
