...
#[requires(correct_data(node, array, nodeLIdx, nodeRIdx))]
...
#[requires(sum_property(node))]
#[requires(leaves_count(node) == rIdx - lIdx)]
#[ensures(array_tree_equivelant(node, array, lIdx, rIdx) ==> node.v() == array_range_sum(array, lIdx, rIdx) && result)]
fn correct_data(node: &Tree, array: &VecWrapperI32, lIdx: isize, rIdx: isize) -> bool {
    if is_leaf(node) {
        node.v()  == array_range_sum(array, lIdx, rIdx)
    } else {
        let mut result = true;
        let mid = (rIdx + lIdx) / 2;
        ...
        match tL {
            None => {}
            Some(box l) => {
                result &= correct_data(l, array, lIdx, mid);
            }
        }
        ...
        match tR {
            None => {}
            Some(box r) => {
                result &= correct_data(r, array, mid, rIdx);
            }
        }
        result && node.v() == array_range_sum(array, lIdx, rIdx)
    }
}
...
#[ensures(correct_data(&result, array, lIdx, rIdx))]
