...
#[pure]
#[ensures(result ==> node.v() >= 0)]
fn max_property(node: &Tree) -> bool {
    if is_leaf(node) {
        node.v() >= 0
    } else {
        let mut leaves = 0;
        ...
        match tR {
            None => {}
            Some(box r) => {
                result &= max_property(r);
                leaves = max(r.v(), leaves);
            }
        }
...
fn max_of_leaves(
    ...
    if is_leaf(node) {
        ...
        assert!(node.v() == array_range_max(array, gLIdx, gRIdx));
        node.v()
    } else {
        ...
        if lIdx < midIdx && rIdx > midIdx  {
            assert!(resultL == array_range_max(array, gLIdx,  mid));
            assert!(resultR == array_range_max(array, mid,  gRIdx));
        } else if lIdx < midIdx{
            assert!(resultL == array_range_max(array, gLIdx,  gRIdx));
            assert!(resultR == 0);
        }else if rIdx > midIdx{
            assert!(resultR == array_range_max(array, gLIdx,  gRIdx));
            assert!(resultL == 0);
        } else {
            assert!(false);
        }
        ...
        let result = max(resultL, resultR);

        assert!(result == array_range_max(array, gLIdx, gRIdx));
        result
    }
}
...
#[requires(forall(|i: isize| (i >= 0 && i <  array.len()) ==> array.lookup(i) >= 0))]
#[requires(array_tree_equivelant(node, array, nodeLIdx, nodeRIdx))]
#[ensures(result == max_of_leaves(node, lIdx, rIdx, leavesCount, gLIdx, gRIdx, array, nodeLIdx, nodeRIdx))]
#[ensures(result == array_range_max(array, gLIdx, gRIdx))]
fn range_max(
    node: &Tree,
    lIdx: isize,
...
fn build(array: &VecWrapperI32, lIdx: isize, rIdx: isize) -> Tree {
    if lIdx == rIdx - 1 {
        ...
        let mut result = Tree::new_leaf(max(left.v(), right.v()));
...
#[requires(forall(|i: isize| (i >= 0 && i <  array.len()) ==> array.lookup(i) >= 0))]
#[ensures(forall(|i: isize| (i > lIdx && i < rIdx) ==> result == max(array_range_max(array, lIdx, i), array_range_max(array, i, rIdx))))]
fn array_range_max(array: &VecWrapperI32, lIdx: isize, rIdx: isize) -> isize {
    if lIdx == rIdx - 1 {
        array.lookup(lIdx)
    } else {
        max(array.lookup(lIdx), array_range_max(array, lIdx + 1, rIdx))
    }
}
...
#[requires(lIdx >= 0 && rIdx <= array.len() && lIdx < rIdx)]
#[requires(forall(|i: isize| (i >= 0 && i <  array.len()) ==> array.lookup(i) >= 0))]
#[ensures(result == array_range_max(array, lIdx, rIdx))]
fn solve(array: &VecWrapperI32, lIdx: isize, rIdx: isize) -> isize {
    let segTree = build(array, 0, array.len());
    range_max(&segTree, lIdx, rIdx,  array.len(), lIdx, rIdx, array, 0, array.len())
}
