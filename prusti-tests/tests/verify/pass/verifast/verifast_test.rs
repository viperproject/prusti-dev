...
fn main() {
    ...
    let r = unsafe { functions::mangle(&mut p) };
    
    prusti_assert_eq!(r, r_val);
    prusti_assert_eq!(p.x, new_x_val);
    prusti_assert_eq!(p.y, new_y_val);

    let p = functions::Point { x: a, y: b };
    let r = unsafe { functions::squared_magnitude(&p) };
