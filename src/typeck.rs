...
use specifications::{
    ...
    Assertion, Expression, ForAllVars, TypedTriggerSet,
    UntypedTriggerSet, Trigger,
};
...
fn type_trigger_set(trigger_set: UntypedTriggerSet,
                    typed_expressions: &HashMap<ExpressionId, rustc::hir::Expr>
                    ) -> TypedTriggerSet {
    TypedTriggerSet::new(
        trigger_set.into_iter().map(|trigger| {
            Trigger::new(
                trigger.into_iter().map(|term| {
                    Expression {
                        id: term.id,
    ...
    Assertion {
        kind: box {
            ...
            match assertion_kind {
                ...
                AssertionKind::Implies(expression, assertion) => {
                    AssertionKind::Implies(
                        Expression {
                            id: expression.id,
                            expr: typed_expressions[&expression.id].clone(),
                        },
                ...
                AssertionKind::ForAll(vars, trigger_set, filter, body) => {
...
impl <'a, 'tcx: 'a> TypeCollector<'a, 'tcx> {
    ...
    fn register_typed_args(&mut self,
                           args: &syntax::ptr::P<[rustc::hir::Expr]>) {
