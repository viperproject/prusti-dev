...
pub(crate) fn build_translator<'a>(
    ...
    match properties.get(&Property::Translator) {
        Some(translator) => build_translator_inner(translator),
        None => None,
    }
}

fn build_translator_inner<'a>(translator: &str) -> Option<Box<dyn SpecTranslator + 'a>> {
    if translator.eq_ignore_ascii_case("\"verifast\"") {
        Some(Box::new(VeriFastTranslator))
    } else {
...
impl From<SpecItemType> for AssertionKind {
    fn from(spec_item_type: SpecItemType) -> Self {
        match spec_item_type {
            ...
            _ => panic!(
                "Invalid spec item type for path state: {:?}",
                spec_item_type
            ),
        }
    }
...
impl VerifastTranslatorContext {
    ...
    fn translate(
        self,
        span: Span,
        path_state_type: AssertionKind,
    ) -> syn::Result<Vec<syn::Attribute>> {
        self.parse(path_state_type)?
            .generate_result(span, path_state_type)
    }
        ...
        while self.pos < self.source.len() {
            ...
            let next_token = match (&self.source[self.pos - 1], self.source.get(self.pos)) {
                (TokenTree::Punct(p1), Some(TokenTree::Punct(p2))) => {
                    ...
                    Some(
                        match (p1.as_char(), p2.as_char()) {
                            ...
                            ('!', '=') => "!=",
                            _ => todo!(
                                "Unsupported punctuation combination for Verifast translation {}{}",
                                p1,
                                p2
                            ),
                        }
                ...
                (TokenTree::Ident(ident), _)
                    if matches!(ident.to_string().as_str(), "true" | "false") =>
                {
                    Some(ident.to_string())
                }
                (TokenTree::Ident(ident), Some(TokenTree::Group(group)))
                    if ident.to_string().as_str() == "old" =>
                {
                ...
                (t, _) => todo!("Unsupported token for Verifast translation {}", t),
            };
    ...
    fn parse_path(&mut self, path_state_type: AssertionKind, start: &proc_macro2::Ident) -> String {
        ...
        loop {
            match (self.source.get(self.pos), self.source.get(self.pos + 1)) {
                (Some(TokenTree::Punct(p)), Some(TokenTree::Ident(ident)))
                    if p.as_char() == '.' =>
                {
                    self.pos += 2;
                    path.push(ident);
        ...
        if path.len() == 1 {
            ...
            let value_identifier = format!(
                "{}{}",
                map_to_prefix(path_state_type),
                path.iter().join("_")
            );
    ...
    fn extend(&mut self, other: Self) {
        self.pre_path_permission_conjunctions
            .extend(other.pre_path_permission_conjunctions.into_iter());
        self.post_path_permission_conjunctions
            .extend(other.post_path_permission_conjunctions.into_iter());
        self.assertion_parts
            .extend(other.assertion_parts.into_iter());
    }

    fn generate_permission_conjunctions(
        &self,
        permission_conjunctions: &BTreeMap<String, String>,
    ) -> String {
        permission_conjunctions
            .iter()
    ...
    fn generate_result(
        ...
        let parsed_post_permission_conjunctions =
            self.generate_permission_conjunctions(&self.post_path_permission_conjunctions);
        let parsed_pre_permission_conjunctions =
            self.generate_permission_conjunctions(&self.pre_path_permission_conjunctions);
        let assertion = self.assertion_parts.into_iter().join(" ");

        Ok(vec![
            parse_quote_spanned! {span=>
                #[prusti::translated_spec_collector = "verifast"]
            },
            parse_quote_spanned! {span=>
...
impl SpecTranslator for VeriFastTranslator {
    fn translate_spec(
        &self,
        span: Span,
        kind: SpecItemType,
        ...
        VerifastTranslatorContext::new(spec.into_iter().collect()).translate(span, kind.into())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_simple_path() {
        test_post("a.b.c == 1", "a->b->c |-> ?_post_a_b_c", "_post_a_b_c == 1");
    }
    ...
    fn test_post_condition_translation(
        ...
        let result = translator
            ...
            .unwrap();
        assert_meta_value_attribute(&result[0], "verifast");
        assert_meta_value_attribute(&result[1], pre_call_bindings);
        assert_meta_value_attribute(&result[2], post_call_bindings);
        assert_meta_value_attribute(&result[3], postcondition);
    }
