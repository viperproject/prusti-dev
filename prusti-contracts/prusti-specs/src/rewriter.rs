use crate::{
    common::HasSignature, properties, spec_translator, specifications::{
...
impl AstRewriter {
    ...
    pub fn process_translatable_assertion<T: HasSignature + Spanned>(
        ...
        let translated_specs = spec_translator::build_translator(&props)
            .map(|translator| translator.translate_spec(item.span(), spec_type.clone(), tokens.clone()))
            .unwrap_or(Ok(Vec::new()))?;
        Ok((vec![self.generate_spec_item_fn(spec_type, spec_id, parse_prusti(tokens)?, item)?], translated_specs))
    }
