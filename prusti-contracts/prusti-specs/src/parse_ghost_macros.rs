...
pub(crate) struct WithFinally {
    ...
    pub finally_block_at_panic_start: syn::Block,
    pub finally_block_at_resume: syn::Block,
}
...
impl Parse for WithFinally {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        let finally_block_at_panic_start = input.parse()?;
        let finally_block_at_resume = input.parse()?;
        Ok(Self {
