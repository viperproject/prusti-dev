/// Each atomic Prusti assertion (`A`) is a Rust expression (`E`) or a `forall` expression.
/// Atomic Prusti assertions can be joined together using the following two operators, forming
/// Prusti assertions:
/// - `A && A` (conjunction)
/// - `A ==> A` (implication)
///
...
///
/// Parentheses can be used as usual, i.e. `(A ==> A) && A` is a Prusti assertion.
///
...
/// `(E ==> E) || E`
///
/// However, this is fine, as `&&` is also a Rust operator, and this whole thing is interpreted as
/// a Rust expression:
/// `(E && E) || E`
///
/// Having `&&` and `||` in the same subexpression is not allowed to prevent ambiguities. Those
/// are all illegal:
/// `E && E || E`
/// `A && (E && E || E)`
///
